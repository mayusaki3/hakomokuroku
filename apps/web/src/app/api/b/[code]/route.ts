...
export async function GET(req: Request, { params }: { params: { code: string } }) {
  const url = new URL(req.url); const t = Number(url.searchParams.get('t')||'0');
  const sig = url.searchParams.get('sig') || '';
  const secret = process.env.QR_SIGN_SECRET || '';

  if (!t || !sig || !secret) return NextResponse.json({ error: 'invalid' }, { status: 400 });
  const now = Math.floor(Date.now()/1000);
  if (Math.abs(now - t) > ALLOW_SKEW) return NextResponse.json({ error: 'expired' }, { status: 403 });
  ...
  // ここで箱ページへサーバーリダイレクトしてもよい（SSR表示等）
  return NextResponse.json({ ok: true, code: params.code, boxId: box.id });
}
