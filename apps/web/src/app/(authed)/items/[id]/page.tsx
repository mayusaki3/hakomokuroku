...
export default function ItemDetailPage() {
  ...
  // 画像（live）— 並び順に追従
  const { data: imgsLive } = useDexieLive(
    async () => {
    ...
    try {
      ...
      for (const f of Array.from(files)) {
        ...
        payload.push({
          ...
          w: full.w,
          h: full.h,
          exif: { orientation: full.orientation ?? thumb.orientation },
        });
      }
  ...
  const onDeleteItem = async () => {
    ...
    if (boxId) router.push(`/boxes/${boxId}/items`);
  ...
  return (
    <main style={{ padding: 24 }}>
      ...
      <div style={{ color: '#555', marginBottom: 12 }}>
        {boxId ? <a href={`/boxes/${boxId}/items`}>← 箱のアイテム一覧に戻る</a> : null}
      </div>
      ...
      <section style={{ display: 'grid', gap: 8, maxWidth: 800, marginBottom: 16 }}>
        ...
        <div>
          <b>メモ：</b>
          <pre style={{ whiteSpace: 'pre-wrap', margin: 0 }}>{note}</pre>
        </div>
      </section>
            ...
            >
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => move(i, -1)} disabled={i === 0} title="上へ">
                    ↑
                  </button>
                  <button onClick={() => move(i, +1)} disabled={i === rows.length - 1} title="下へ">
                    ↓
                  </button>
                  <button onClick={() => onDeleteImage(r.id)} style={{ color: '#b91c1c' }}>
                    削除
                  </button>
                </div>
              </figcaption>
...
function MoveItemButton({ itemId }: { itemId: string }) {
  ...
  const onMove = async () => {
    const key = prompt(
      '移動先の「箱ID」または「箱コード」を入力してください（例: UNASSIGNED または B-...）'
    );
    if (!key) return;
