...
export default function ThemeEditorPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (mode !== 'new' && editId) {
          const r = await fetch(`/api/settings/theme/get?id=${encodeURIComponent(editId)}`, {
            cache: 'no-store',
          });
  ...
  const Row = ({ label, children }: any) => (
    <div
      style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 8, alignItems: 'center' }}
    >
    ...
    <>
      <ContentCard>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn" onClick={save}>
              <Save size={16} style={{ marginRight: 6 }} /> 保存
            </button>
            <button className="btn" onClick={cancel}>
              <X size={16} style={{ marginRight: 6 }} /> キャンセル
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <Row label="壁紙（アップロード/撮影）">
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <button className="btn" onClick={onPickFile}>
                <ImageIcon size={16} style={{ marginRight: 6 }} /> 画像
              </button>
              <button className="btn" onClick={onPickCamera}>
                <Camera size={16} style={{ marginRight: 6 }} /> カメラ
              </button>
              {draftWall && (
                <>
                  <img
                    src={draftWall}
                    alt="thumb"
                    style={{ height: 36, borderRadius: 4, border: '1px solid var(--hk-border)' }}
                  />
                  <button className="btn" onClick={onClearWallpaper}>
                    <Trash2 size={16} style={{ marginRight: 6 }} /> クリア
                  </button>
                </>
              )}
              <input
          ...
          <Row label="ヘッダ文字色">
            <ColorInput varName="hk-header-fg" />
          </Row>

          <HR />
          <Row label="ツールバー 背景色">
            <ColorInput varName="hk-toolbar-bg" />
          </Row>
          <Row label="ツールバー 文字色">
            <ColorInput varName="hk-toolbar-fg" />
          </Row>

          <HR />
          <Row label="コンテンツ 背景色">
            <ColorInput varName="hk-content-bg" />
          </Row>

          <HR />
          <Row label="入力 背景色">
            <ColorInput varName="hk-input-bg" />
          </Row>
          <Row label="入力 枠線色">
            <ColorInput varName="hk-input-border" />
          </Row>
          <Row label="入力 文字色">
            <ColorInput varName="hk-input-fg" />
          </Row>

          <HR />
          <Row label="ボタン 背景色">
            <ColorInput varName="hk-btn-bg" />
          </Row>
          <Row label="ボタン 枠線色">
            <ColorInput varName="hk-btn-border" />
          </Row>
          <Row label="ボタン 文字色">
            <ColorInput varName="hk-btn-fg" />
          </Row>
