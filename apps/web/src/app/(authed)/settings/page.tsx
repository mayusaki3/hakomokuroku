...
export default function SettingsHomePage() {
  ...
  const onChangeActive = async (id: string) => {
    ...
    const vars = id
      ? await fetch(`/api/settings/theme/get?id=${encodeURIComponent(id)}`, { cache: 'no-store' })
          .then((r) => r.json())
          .then((j) => j?.theme?.vars || {})
      : {};
  ...
  async function loadAndApplyTheme(id: string) {
    ...
    try {
      const r = await fetch(`/api/settings/theme/get?id=${encodeURIComponent(id)}`, {
        cache: 'no-store',
      });
      if (!r.ok) {
      ...
      <div className="app-scroll">
        ...
        <Section title="スキャン">
          <label className="row" style={{ alignItems: 'center' }}>
            <input
              type="checkbox"
              checked={s.preferBackCamera}
              onChange={(e) => update('preferBackCamera', e.target.checked)}
            />
            背面カメラを優先する
          </label>
          <label className="row" style={{ alignItems: 'center', marginTop: 6 }}>
            <input
              type="checkbox"
              checked={s.scanBeep}
              onChange={(e) => update('scanBeep', e.target.checked)}
            />
            読み取り時に音/バイブ
          </label>
        </Section>
        ...
        <Section title="検索/絞り込み">
          <label className="row" style={{ alignItems: 'center' }}>
            <input
              type="checkbox"
              checked={s.keepFiltersOnNav}
              onChange={(e) => update('keepFiltersOnNav', e.target.checked)}
            />
          ...
          <div className="row" style={{ alignItems: 'center', marginTop: 8 }}>
            ...
            <select
              value={s.sortDefault}
              onChange={(e) => update('sortDefault', e.target.value as any)}
            >
        ...
        <Section title="ラベル/QR 表示">
          ...
          <div style={{ marginTop: 8 }}>
            <Link href="/labels" className="btn-link">
              ラベル出力ページへ
            </Link>
          </div>
        </Section>
        ...
        <Section title="バックアップ/リストア">
          <div>
            <Link href="/settings/backup" className="btn-link">
              全データのバックアップ/リストアへ
            </Link>
          </div>
        </Section>
