...
export default function VisionSettingsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setProvider(j.provider || 'none');
      setApiKey(j.apiKey || '');
      setPName(j.promptName || '');
      setPTags(j.promptTags || '');
      setPNote(j.promptNote || '');
    })();
  }, []);

  const save = async () => {
    setErr('');
    const r = await fetch('/api/settings/vision/save', {
      method: 'POST',
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <Row label="テスト画像">
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <button className="btn" onClick={() => onPick('image/*', 'vision-file')}>
                ファイル選択
              </button>
              <button
                className="btn"
                onClick={() => onPick('image/*;capture=camera', 'vision-camera')}
              >
                カメラ
              </button>
              {thumb && (
                <img
                  src={thumb}
                  alt="thumb"
                  style={{ height: 48, border: '1px solid var(--hk-border)', borderRadius: 4 }}
                />
              )}
              <input
                id="vision-file"
          ...
          <Row label="結果（名前）">
            <input value={resName} readOnly style={{ width: '100%' }} />
          </Row>
          <Row label="結果（タグ）">
            <input value={resTags} readOnly style={{ width: '100%' }} />
          </Row>
          <Row label="結果（メモ）">
            <textarea value={resNote} readOnly rows={3} style={{ width: '100%' }} />
          </Row>
