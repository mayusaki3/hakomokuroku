...
次の設計判断点:
**オフライン起動時に、どのユーザー専用IndexedDBを開くかをどう決めるか。**

候補:
1. 最後に正常ログインした `User.id` を端末ローカルの小さなユーザー選択情報として保持し、オフライン時はそのユーザーDBを開く。
2. オフライン時はユーザーDBを自動で開かず、保存済みユーザー一覧から選択させる。
3. オフライン時はログイン済みセッションが検証できないため利用不可とする。

PWAのオフライン利用を維持するなら1または2が必要。複数ユーザー利用を考慮すると、推奨は「最後のユーザーを既定選択しつつ、保存済みユーザーが複数なら切替可能」とする方式。
