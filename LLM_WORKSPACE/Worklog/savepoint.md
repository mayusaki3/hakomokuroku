...
- 同一User内のみblob再利用可能。hash方式変更は明示的な全体migrationとする
...
規則:
- client UIは11枚目を追加させない
- serverもentity payloadの写真配列が10件を超える場合はREJECTEDとする
- 写真順序は保持し、先頭写真を代表画像として利用可能
- 上限はlocal/online/offline/backup/restore/Push/Pull/Full Resyncで共通
- restore/importデータも10枚上限を満たす必要がある

根拠: thumbnailをentity JSONへ埋め込むため無制限ではPull/Full Resync payloadとlocal storageが無制限に増える。箱・物品・置き場所の記録用途では10枚あれば十分な余裕があり、全entity共通値にすることでUI/API/validation/testを単純化できる。

### PhotoBlob GC
- 未参照状態30日継続で物理削除可能。`unreferencedSince`はserver管理時刻
- reference countを正本にせず、Box/Item/BoxLocation canonicalの実参照を正本とする
- GC直前に同一User内の実参照ゼロを再確認し、再参照時は`unreferencedSince=null`
