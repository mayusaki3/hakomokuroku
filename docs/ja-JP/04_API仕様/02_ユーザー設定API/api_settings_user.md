...
[目次](../../目次.md) > API仕様 > ユーザー設定API > ユーザー設定取得・更新（GET/PUT /api/settings/user）
...
| sec_settings_user_put_parse_body | PUT JSON parse | JSON不正時は 400 を返す |
| sec_settings_user_put_validate_display_name | PUT 入力検証 | displayName 未指定/空/非string は 400 を返す |
...
- HTTP 200
- `prisma.user.update({ where: { id }, data: { userName: displayName } })` を実行する
